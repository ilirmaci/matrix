...
def morph(S, B):
    ...
    for z in B:
        w = exchange(S_copy, protected, z)
        S_copy.append(z) # vector to be injected from B into S
        protected.append(z) # remember injected vector from B
        S_copy.remove(w) # vector to be ejected from S
        veclist.append((z, w))
    return veclist
...
def subset_basis(T): 
    ...
    i = 0    # initializing index for list T
    S = T.copy() # creating copy of T
    
    
    while i < len(S):
        if is_superfluous(S, i):
            S = S[:i] + S[i+1:] #remove entry i
        else:
            i += 1
    return S
...
def find_matrix_inverse(A):
    ...
    A_rowspace = A.D[0]
    A_nrows = len(A_rowspace)
    
    for (r, i) in zip(A_rowspace, range(A_nrows)):
        v = Vec(A_rowspace, {i:one}) # corresponding col of I matrix
        b = solve(A, v)
