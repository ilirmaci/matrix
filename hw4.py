...
indep_vec_4 = Vec({0, 1, 2}, {k:1 for k in range(3)})
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    u = vec2rep(S, z) # finding coefficients of z in terms of S[i]
	
    for (a, vec) in zip(u, S):
        if a != 0 and not vec in A:
            return vec
