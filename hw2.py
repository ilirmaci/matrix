...
def vec_sum(veclist, D): 
    ...
    True
    '''
    #return Vec(D, {x:sum([l[x] for l in veclist]) for x in D})
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [vecdict[k]/k for k in vecdict]
...
def GF2_span(D, L):
    ...
    True
    '''
    # make into recursive procedure that returns span{L[1:]}
    # and L[0]+span{veclist[1:]}
    
    if len(L) <= 1:
        return [Vec(D, {}), L[0]]
    list_so_far = GF2_span(D, L[1:])
    for x in list_so_far:
        new = L[0] + x
        if new not in list_so_far:
            list_so_far.append(new)
    #list_so_far.extend([L[0]+x for x in list_so_far if L[0]+x not in list_so_far])
    return list_so_far
...
if __name__ == "__main__":
    ...
    #from GF2 import one
    #D = {'a', 'b'}
    #A = GF2_span(D, [Vec(D, {'a':one, 'b':0}), Vec(D, {'a':0, 'b':one}), Vec(D, {'a':one, 'b':one})])
    #for x in A:
        #print(x)
